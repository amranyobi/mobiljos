...
class Spare extends CI_Controller {
	...
	public function index()	{
		...
		$spare = $this->spare_model->listing();
		$data = array(	'title'			=> 'Data Bengkel & Sparepart',
						'spare'		=> $spare,
						'isi'			=> 'admin/spare/list');
	...
	public function tambah()	{
		...
		$valid->set_rules('nama_spare','Judul','required',
			array(	'required'	=> 'Nama Bengkel harus diisi'));
		if($valid->run()) {
			$fileExt = pathinfo($_FILES["gambar1"]["name"], PATHINFO_EXTENSION);
			$image = time().'8.'.$fileExt;
			...
			$this->load->library('upload', $config);
      		...
      		if(! $this->upload->do_upload('gambar1')) {
				// End validasi
				$data = array(	'title'				=> 'Tambah Bengkel',
								'error'    			=> $this->upload->display_errors(),
								'isi'				=> 'admin/spare/tambah');
				$this->load->view('admin/layout/wrapper', $data, FALSE);
				// Masuk database
			}else{
		        ...
		        redirect(base_url('admin/spare'),'refresh');
			}}
		// End masuk database
		$data = array(	'title'				=> 'Tambah Bengkel',
						'isi'				=> 'admin/spare/tambah');
	...
	// Delete
	public function delete($id_spare) {
		// Tambahkan proteksi halaman
		$spare = $this->spare_model->detail($id_spare);
		// Proses hapus gambar
		if($spare->file=="") {
		}else{
			unlink('./assets/upload/image/'.$spare->file);
			unlink('./assets/upload/image/thumbs/'.$spare->file);
		}
		// End hapus gambar
		$data = array('id_spare'	=> $id_spare);
		$this->spare_model->delete($data);
		$this->session->set_flashdata('sukses', 'Data telah dihapus');
		redirect(base_url('admin/spare'),'refresh');
	}
}
