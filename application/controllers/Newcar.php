...
class Newcar extends CI_Controller {
	...
	public function index()	{
		...
		$config['base_url'] 		= base_url().'newcar/index/';
		$config['total_rows'] 		= count($this->newcar_model->total_galeri());
		$config['use_page_numbers'] = TRUE;
		$config['num_links'] 		= 5;
		$config['uri_segment'] 		= 3;
		$config['per_page'] 		= 12;
		$config['first_url'] 		= base_url().'newcar/';
		$this->pagination->initialize($config); 
		$page 		= ($this->uri->segment(3)) ? ($this->uri->segment(3) - 1) * $config['per_page'] : 0;
		$galeri 	= $this->newcar_model->galeri($config['per_page'], $page);
		// End paginasi

		$data = array(	'title'		=> 'Galeri Mobil Baru - '.$site->namaweb,
						'deskripsi'	=> 'Galeri Mobil Baru - '.$site->namaweb,
						'keywords'	=> 'Galeri Mobil Baru - '.$site->namaweb,
