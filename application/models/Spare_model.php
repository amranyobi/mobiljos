...
class Spare_model extends CI_Model {
	...
	public function galeri($limit,$start) {
		...
		$this->db->from('spare');
		// Join dg 2 tabel
		$this->db->order_by('id_spare','DESC');
	...
	// Detail data
	public function detail($id_spare) {
		$this->db->select('*');
		$this->db->from('spare');
		$this->db->where('id_spare',$id_spare);
		$this->db->order_by('id_spare','DESC');
		$query = $this->db->get();
		return $query->row();
	}
	...
	public function delete($data) {
		$this->db->where('id_spare',$data['id_spare']);
		$this->db->delete('spare',$data);
	}
}
